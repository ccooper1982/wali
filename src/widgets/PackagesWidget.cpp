...
void PackagesWidget::search ()
{
  ...
  for (const auto it : package_names | view::split(' '))
  {
    ...
    // ignore if: request already sent, already confirmed or added on a previous visit to the page
...
/// Throttle requests:
/// Can't find the actual rate limit, so to avoid rate limit response and connection resets,
/// send 3 requests, 200ms apart.
void PackagesWidget::send_next()
{
  ...
  while (m_queue.size() && scheduled < MaxActiveRequests)
  {
    ...
    {
      // TODO should be removing this child when done?
      ...
      {
        --m_responses_expected;
        m_packages_pending.erase(package);
      }
    });
...
{
  const bool error = rsp_err || rsp.status() != 200;
  if (error)
  {
  ...
  {
    // TODO should use the root_object["version"] (currently 2)
    Json::Value root;
    ...
    {
      if (const Json::Array results = root_object.get("results"); results.empty())
        --m_responses_expected;
      else
      {
  ...
  if (error || m_packages_confirmed.size() + m_packages_pending.size() == m_responses_expected)
  {
    m_line_packages->setText(flatten(m_packages_pending));
    m_packages_pending.clear(); // for when there an error
    m_btn_search->enable();
